...
function subPass() {
  let currentPass = parseInt(passButt.innerText);
  if (currentPass > 0) {
    passButt.innerText = currentPass - 1;
  }
}
function addPass() {
  let currentPass = parseInt(passButt.innerText);
...
function addPoints() {
  let currentPoints = parseInt(point.innerText);
  point.innerText = currentPoints + 1;
}
...
addButt.addEventListener("click", addPoints);
subButt.addEventListener("click", subPoints);
passButt.addEventListener("click", subPass);
...
document.addEventListener("keydown", function (e) {
  if (e.which == 32) {
    if (!isTimerRunning) {
      ...
      if (parole.length > 0) {
        const genWord = Math.floor(Math.random() * parole.length);
        const genWord1 = parole[genWord];
        parole.splice(genWord, 1);
        document.getElementById("parole").innerText = genWord1;
      } else {
        document.getElementById("parole").innerText = "FINE PAROLE";
      }
    } else {
      stopTimer();
    }
